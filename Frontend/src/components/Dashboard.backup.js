...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      let statsRes, collaborationsRes, postsRes;
      
      try {
        console.log('📊 Dashboard: Fetching stats...');
        statsRes = await apiService.getStats();
        console.log('✅ Dashboard: Stats fetched successfully', statsRes);
      } catch (statsError) {
      ...
      try {
        console.log('🤝 Dashboard: Fetching collaborations...');
        collaborationsRes = await apiService.getCollaborations();
        console.log('✅ Dashboard: Collaborations fetched successfully', collaborationsRes);
      } catch (collabError) {
        console.error('❌ Dashboard: Collaborations fetch failed', collabError);
        throw new Error(`Collaborations API failed: ${collabError.message}`);
      }

      try {
        console.log('📝 Dashboard: Fetching posts...');
        postsRes = await apiService.getPosts();
        console.log('✅ Dashboard: Posts fetched successfully', postsRes);
      } catch (postsError) {
        console.error('❌ Dashboard: Posts fetch failed', postsError);
        throw new Error(`Posts API failed: ${postsError.message}`);
      }
    ...
    <div>
      ...
      <div className="dashboard-grid">
        ...
        <div className="dashboard-card">
          ...
          <div style={{ marginTop: '1rem' }}>
            <p>Total donors: {posts.reduce((sum, post) => sum + (post.donations || 0), 0)}</p>
          </div>
        </div>
